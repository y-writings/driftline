...
driftline's current TOML redesign makes the current Contract and current Sync manifest the only sources of truth. `driftline update` must apply a Sync plan without writing anything when conflicts exist, must update Managed files, must remove stale Managed files, and must rewrite the Sync manifest only when the Managed set changes.

The current `runUpdate` command owns too much Target Repository behavior. It reads `Change` status and flags, deletes files, writes files, prepares the Sync manifest rewrite, commits the rewrite, and prints the report. This makes the command Module shallow: the CLI Adapter knows the ordering rules that make Target Repository writes safe.
...
2. Prepare a Sync manifest rewrite only when the Sync plan includes Sync manifest additions or removals.
3. Delete stale Managed file targets first.
4. Write added or updated Managed file targets second.
5. Commit the Sync manifest rewrite last.
...
- Sync manifest commit happens after all Managed file deletes and writes succeed.
- If a Managed file delete or write fails, the Sync manifest is not committed.
...
The deliberately limited guarantee is: the Sync manifest must not move ahead of file operations. Full transactional rollback is out of scope because it would require preserving and restoring file content, symlink state, permissions, directory state, and partial filesystem failures.
...
- File-only updates do not rewrite an unchanged Sync manifest.
- Managed-to-template transitions remove the Sync manifest entry and leave the target file untouched.
...
- Applying a conflict plan returns an error before writing files or committing the Sync manifest.
- Applying a plan that removes stale `dir` and writes `dir/file` succeeds because deletes happen before writes.
- If a Managed file write fails, the Sync manifest is not committed.
- If a plan has only Managed file content changes, the Sync manifest is not rewritten.
- `runUpdate` still preserves current CLI output by printing from the Sync plan after apply.

Existing command integration tests should remain useful, but the ordering and Sync manifest commit guarantees should be testable through the new Module directly.

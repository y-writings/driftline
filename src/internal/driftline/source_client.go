...
func (c *GitHubClient) ResolveDefaultRef(repository string) (string, string, error) {
...
func (c *GitHubClient) ResolveRef(repository string, ref string) (string, error) {
	if err := ValidateRepository(repository); err != nil {
		return "", err
	}
...
func (c *GitHubClient) ReadFile(repository string, commit string, path string) ([]byte, error) {
	if err := ValidateRepository(repository); err != nil {
		return nil, err
	}
	...
	if err := ValidateConfigPath(path, "source"); err != nil {
		return nil, err
	}
...
func (c *GitHubClient) newRequest(method string, endpoint string) (*http.Request, error) {
...
func (c *GitHubClient) httpError(res *http.Response, context string) error {
